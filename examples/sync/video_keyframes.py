...
Example:
  uv run examples/sync/video_keyframes.py
  uv run examples/sync/video_keyframes.py --duration 12 --resolution 480p
"""
...
def main(argv: Sequence[str]) -> None:
    ...
    winter = client.image.sample(WINTER_PROMPT, model=IMAGE_MODEL.value, aspect_ratio="16:9", image_format="url")
    stills = {"winter": winter.url}
    print(f"winter: {stills['winter']}")
    for season, prompt in SEASON_EDITS.items():
        edited = client.image.sample(prompt, model=IMAGE_MODEL.value, image_url=stills["winter"], image_format="url")
        stills[season] = edited.url
        print(f"{season}: {stills[season]}")
