...
def test_invalid_api_key():
    ...
    with server.run_test_server() as port:
        ...
        assert exc_info.value.code() == grpc.StatusCode.UNAUTHENTICATED  # type: ignore
...
def test_server_error():
    ...
    with server.run_test_server(initial_failures=1) as port:
        ...
        with pytest.raises(grpc.RpcError) as exc_info:
            client.auth.get_api_key_info()
        assert exc_info.value.code() == grpc.StatusCode.UNAVAILABLE  # type: ignore
...
def test_timeout(mock_tracer):
    ...
    rpc_error.code = lambda: grpc.StatusCode.DEADLINE_EXCEEDED  # type: ignore
    rpc_error.details = lambda: "Deadline Exceeded"  # type: ignore
    ...
    assert exc_info.value.code() == grpc.StatusCode.DEADLINE_EXCEEDED  # type: ignore
